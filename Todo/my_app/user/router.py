...
def get_user(id: int, service: UserService = Depends(get_user_service), token: HTTPAuthorizationCredentials = Depends(security)):
    return service.get_user(id)

# DELETE
@router.delete("", response_model=None)
def delete_user(id: int, service: UserService = Depends(get_user_service), token: HTTPAuthorizationCredentials = Depends(security)):
